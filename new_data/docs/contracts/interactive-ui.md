...
Production vault decryption password prompt (dialog --passwordbox).
Install mode selector: minimal, server, desktop (dialog --menu).
Main task selection checkboxes (dialog --checklist).
Force-mode question: Yes / No, default No (dialog --menu or dialog --yesno).
Force-task checkboxes, shown only when force-mode is Yes (dialog --checklist).
...
Bootstrap reconnects stdin to /dev/tty before interactive screens.
If /dev/tty is unavailable, bootstrap logs the reason and does not hang.
