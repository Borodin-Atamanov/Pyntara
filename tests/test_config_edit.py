...
def test_sync_directives_protects_a_commented_directive(tmp_path: Path) -> None:
    # A commented directive is a foreign line, not a comment to be dropped
    # by a sign: the sign that marks the comment is part of its key, so a
    # line the operator commented out survives the merge while the exact
    # directive is added next to it.
    from pyntara.config_edit import sync_directives_by_key
