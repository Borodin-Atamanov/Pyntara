...
def make_config(
    *,
    ...
    dnscrypt_package_name: str = "dnscrypt-proxy",
    dnscrypt_config_path: Path = Path("/etc/dnscrypt-proxy/dnscrypt-proxy.toml"),
    dnscrypt_service_unit_name: str = "dnscrypt-proxy.service",
    dnscrypt_socket_unit_name: str = "dnscrypt-proxy.socket",
    dnscrypt_socket_dropin_dir: Path = Path(
        "/etc/systemd/system/dnscrypt-proxy.socket.d"
    ),
    dnscrypt_socket_dropin_file_name: str = "pyntara.conf",
    dnscrypt_socket_dropin_file_mode: int = 0o644,
    dnscrypt_socket_section: str = "[Socket]",
    dnscrypt_socket_dropin_header: str = (
        "# Managed by the Pyntara dnscrypt_setup task."
    ),
    dnscrypt_listen_address: str = "0.0.0.0:53053",
    dnscrypt_fallback_resolvers: tuple[str, ...] = ("1.1.1.1", "8.8.8.8"),
    dnscrypt_resolved_conf_dir: Path = Path("/etc/systemd/resolved.conf.d"),
    dnscrypt_dropin_file_name: str = "dnscrypt.conf",
    dnscrypt_dropin_file_mode: int = 0o644,
    dnscrypt_resolve_section: str = "[Resolve]",
    dnscrypt_dropin_header: str = (
        "# Managed by the Pyntara dnscrypt_setup task."
    ),
    dnscrypt_dns_directive: str = "DNS=127.0.0.1:53053",
    dnscrypt_domains_directive: str = "~.",
    dnscrypt_directive_keys: tuple[str, ...] = ("DNS", "Domains"),
    dnscrypt_manage_networkmanager: bool = True,
    dnscrypt_install_retries: int = 3,
    dnscrypt_start_check_attempts: int = 5,
    dnscrypt_start_check_retry_delay_seconds: float = 1.0,
    ...
    nextdns_domains_directive: str = "~.",
    nextdns_ipv4_servers: tuple[str, ...] = ("45.90.28.0", "45.90.30.0"),
    nextdns_ipv6_prefixes: tuple[str, ...] = ("2a07:a8c0", "2a07:a8c1"),
    nextdns_dot_endpoint_format: str = "{profile_id}.dns.nextdns.io",
    nextdns_verification_url: str = "https://test.nextdns.io/",
    ...
    nextdns_directive_keys: tuple[str, ...] = (
        "DNS",
        "FallbackDNS",
        "DNSOverTLS",
        "Domains",
    ),
    nextdns_nmcli_check_command: tuple[str, ...] = ("nmcli", "--version"),
    nextdns_nmcli_list_command: tuple[str, ...] = (
        "nmcli",
        "-t",
        "-f",
        "NAME",
    ...
    nextdns_nmcli_modify_command: tuple[str, ...] = (
        "nmcli",
        "connection",
        "modify",
        "{connection}",
    ...
    nextdns_restart_resolved_command: tuple[str, ...] = (
        "systemctl",
        "restart",
        "systemd-resolved",
    ),
    ...
    return Config(
        ...
        dnscrypt_setup=DnscryptSetupConfig(
            package_name=dnscrypt_package_name,
            config_path=dnscrypt_config_path,
            service_unit_name=dnscrypt_service_unit_name,
            socket_unit_name=dnscrypt_socket_unit_name,
            socket_dropin_dir=dnscrypt_socket_dropin_dir,
            socket_dropin_file_name=dnscrypt_socket_dropin_file_name,
            socket_dropin_file_mode=dnscrypt_socket_dropin_file_mode,
            socket_section=dnscrypt_socket_section,
            socket_dropin_header=dnscrypt_socket_dropin_header,
            listen_address=dnscrypt_listen_address,
            fallback_resolvers=dnscrypt_fallback_resolvers,
            resolved_conf_dir=dnscrypt_resolved_conf_dir,
            dropin_file_name=dnscrypt_dropin_file_name,
            dropin_file_mode=dnscrypt_dropin_file_mode,
            resolve_section=dnscrypt_resolve_section,
            dropin_header=dnscrypt_dropin_header,
            dns_directive=dnscrypt_dns_directive,
            domains_directive=dnscrypt_domains_directive,
            directive_keys=dnscrypt_directive_keys,
            manage_networkmanager=dnscrypt_manage_networkmanager,
            ...
            start_check_attempts=dnscrypt_start_check_attempts,
            start_check_retry_delay_seconds=dnscrypt_start_check_retry_delay_seconds,
        ),
        ...
        nextdns_setup_system_wide=NextdnsSetupSystemWideConfig(
            ...
            ipv4_servers=nextdns_ipv4_servers,
            ipv6_prefixes=nextdns_ipv6_prefixes,
            dot_endpoint_format=nextdns_dot_endpoint_format,
            verification_url=nextdns_verification_url,
            dns_over_tls=nextdns_dns_over_tls,
            fallback_dns=nextdns_fallback_dns,
            directive_keys=nextdns_directive_keys,
            nmcli_check_command=nextdns_nmcli_check_command,
            nmcli_list_command=nextdns_nmcli_list_command,
            nmcli_modify_command=nextdns_nmcli_modify_command,
            restart_resolved_command=nextdns_restart_resolved_command,
            resolvectl_status_command=nextdns_resolvectl_status_command,
            verification_command=nextdns_verification_command,
