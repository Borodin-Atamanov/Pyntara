...
def _engine():
    """The engine values the report command is built from."""

    return make_config().engine


def test_direct_command_carries_client_verbose_port_and_address(engine) -> None:
    ...
    assert (
        ssh_access.ssh_command(engine, "10.10.0.1", 30222)
        == "ssh -v -p 30222 10.10.0.1"
    )
    assert (
        ssh_access.ssh_command(engine, "example.onion", 22)
        == "ssh -v -p 22 example.onion"
    )


def test_proxy_command_routes_through_the_local_socks_proxy(engine) -> None:
    ...
    assert ssh_access.ssh_command(
        engine, "address.b32.i2p", 30222, "127.0.0.1:4447"
    ) == (
...
def test_proxy_address_is_the_loopback_address_of_the_router(engine) -> None:
    assert ssh_access.socks_proxy_address(engine, 9050) == "127.0.0.1:9050"


def test_link_scope_address_keeps_its_zone(engine) -> None:
    # An IPv6 link scope address is reachable only through its own
    # interface, so the zone index travels with the target.
    assert ssh_access.ssh_command(
        engine, "fe80::b1e1:869:8e81:2526%enp87s0", 30222
