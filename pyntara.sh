...
# PYNTARA_TASKS="add_extra_repos hostname btrfs_setup swapfile_service_install zram_service zswap_service local_vault_setup system_metrics_setup ssh_daemon_setup ssh_client_setup port_forwarding_setup kde_keyboard_setup kde_settings keyring_setup vocalinux_setup nextdns_setup_system_wide dnsproxy_setup i2pd_service_setup yggdrasil_service_setup tor_setup three_x_ui_xray_setup sotavpn_setup rustdesk_setup imagemagick_setup ffmpeg_setup upnp_forwarding_setup system_metrics_initial_collect chrome_setup playwright_setup cli_tools_lite_setup cli_tools_heavy_setup telegram_setup scrcpy_setup btrfs_recompress btrfs_points_setup commit_final_system_metrics"
# PYNTARA_FORCE_TASKS=""
# PYNTARA_SKIP_APT_UPDATE=1

# 1, and an absent value, delete what a task downloaded once it has served its
# purpose; 0 keeps it, so a repeated run reuses the download.
