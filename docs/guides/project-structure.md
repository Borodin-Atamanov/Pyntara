...
src/pyntara/upnp.py — Shared UPnP port-forwarding helpers over the external upnpc client: reads the router address, lists the existing mappings, asks the router for a mapping and reads the result back; forward_inbound_port runs the whole attempt and returns the router address only when it can work (docs/spec/3x-ui.md). Imported by three_x_ui_xray_setup.
