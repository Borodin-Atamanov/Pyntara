...
The four secret files (default/production vaults and their passwords) are listed in [Secrets files](../contracts/bootstrap.md#12-secrets-files); their structure is described in [Secrets model](../spec/secrets-model.md).
...
Modules planned but not implemented yet are listed in [What is next](../simplified-architecture.md#5-what-is-next-separate-changes).
