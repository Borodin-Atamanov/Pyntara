...
src/pyntara/forwarding_ports.py — The shared deterministic port chain of a machine: desired_port is the port of one name, derived from sha256 of the name mapped into the configured range, and candidate_ports yields the candidates of the chain in the order they are tried, so the reverse tunnel service and the router service can never disagree about the number that names a machine (docs/spec/port-forwarding-setup.md, docs/spec/upnp-forwarding-setup.md). Imported by port_forwarding and upnp_forwarding.
