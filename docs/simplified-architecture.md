...
3. Single config source added as the source of truth for the Python part. The config/ directory at the repository root, one TOML file per top-level section, holds the engine values and the task catalog: task data root, notice timeout, per-task data such as the cli_tools package list, and one [[tasks]] entry per task with name, description, dependencies and mode membership. The loader joins the files into a single document; a missing or invalid config stops the run, there are no defaults. Environment variables remain the inst.sh interface for per-run selection (mode, tasks, force) and secrets. Env-over-config priority is deferred. Invalid environment values never stop the run: they follow the resilience rule (section 7).
