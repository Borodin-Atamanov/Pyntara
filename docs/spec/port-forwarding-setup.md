...
The server addresses come from the vault group port_forwarding_servers, one entry per server with the address in the url field. The address may be ipv4, ipv6 or a url; entries without an url are skipped. A server whose address is a local address of the machine itself, taken from ip -o addr at service start, is skipped: the machine never tunnels onto itself, and when ip is unavailable the machine keeps every server, erring toward forwarding. Both sides are normalized to the canonical address form through ipaddress, so any written variant of the same address is caught: case, leading zeroes, :: compression, embedded ipv4 and the interface zone do not matter. The group is data, like the NextDNS accounts: the regeneration tooling creates it, fills it with its seed entries on creation and never edits them afterwards. The default vault carries the seed entry with the well-known test address 200:a804:881c:d5d8:6d4e:afab:e158:371, so a runtime vault derived from it mirrors the production structure while connecting to nothing real; a vault without the group makes the service connect to nothing.

The key passphrase comes from the vault entry ssh_passphase_for_port_forwarding. The entry exists in every vault: the regeneration tooling generates a fresh password of 7 proquint words joined by dashes on creation (generated_password = "proquint-7" in the vault structure), so the production secret is never copied into another vault and the default vault carries a passphrase that unlocks no deployed key. A runtime vault without the entry makes the service connect to nothing.
...
The assigned remote ports live in the root-only state file. The System Metrics collector carries a port_forwarding network module that reads the state file through the single config it is given and reports one JSON record per server and forwarded local port: the server host, the local port, the granted remote port and the ssh command that reaches this machine through that server, where the reverse tunnel delivers the connection to the local SSH daemon. Every network report therefore shows the current ports per server and the command that uses them; a machine without the state file reports an empty module instead of an error. On a granted-port change the service saves the state and triggers a fresh collection through systemctl start --no-block on the collector service, so the network report is regenerated with the new ports and sent through the existing pipeline. The collector's non-blocking flock skips the trigger when a collection is already running; the daily collection still carries the current ports. No separate telemetry file is produced.
...
All parameters live in the [port_forwarding_setup] table of the config/ directory: the vault group and passphrase entry titles, the remote user, the desired port range (the Linux ephemeral range by default), the ssh keepalive and connect options (connect_timeout_seconds for one ssh attempt, own_addresses_timeout_seconds for the ip call that lists this machine's own addresses, agent_start_timeout_seconds and key_unlock_timeout_seconds for the ssh-agent start and the key unlock), the reconnect backoff, the state file name with state_file_mode and the mode of the askpass helper askpass_helper_file_mode with the display it is given askpass_display, the service unit name, the restart pause and the journal identifier. The unit itself is rendered from the template named by service_template_file_name and runs the module named by service_module_name; the four systemctl_* commands drive that unit, each carrying the unit name as its {service_unit_name} placeholder except the daemon reload, and start_check_attempts with start_check_retry_delay_seconds bound the loop that decides whether a started service failed. The interpreter inside the shared deployment venv is composed from system_metrics_setup.venv_dir and system_metrics_setup.venv_python_relative_path, so the layout of that venv lives in one place. The forwarded local port and the port-forwarding key file names come from the ssh_daemon_setup table, the single source of truth.
