...
## Cache and logging
Caching is enabled by default and is an explicit configuration value. The cache size in bytes is a configured value passed through the dnsproxy `--cache-size` flag. The outbound query timeout in seconds is a configured value passed through the dnsproxy `--timeout` flag. dnsproxy writes its own log (startup info and upstream errors) to stdout, which systemd captures into the journal under the service unit; there is no standalone log file, so journald bounds the log volume. The unit carries the configured journal rate limit (LogRateLimitIntervalSec and LogRateLimitBurst), so an outage cannot flood the journal. The production command does not pass `--verbose`, so no per-query line is logged; only startup info and upstream errors reach the journal.

## Installation
The task reads the latest release from the configured GitHub API repository, maps the Debian architecture to the official Linux tar archive, downloads it into the configured directory, extracts and validates the binary, and atomically replaces the installed binary only after the staged binary reports the expected version. The release query and the binary download run with the engine-wide curl settings: curl_timeout_seconds bounds the metadata query, curl_download_timeout_seconds bounds the download, and the retry count, delay, connect timeout and retry window come from the same [engine] table. A failed update does not remove a working installed binary.
...
## Idempotency
A matching installed binary, service unit, resolver drop-in, enabled and active service and successful verification produce a done result with changed=False. Force mode restarts and reapplies the owned state. All task-owned paths and values come from `[dnsproxy_setup]` configuration.
