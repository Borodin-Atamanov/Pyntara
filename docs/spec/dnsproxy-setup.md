...
When NetworkManager is absent the task cannot remove the per-link provider DNS, so the routing check reads the Global block of resolvectl status instead of failing blindly. Two facts prove that systemd-resolved routes queries through dnsproxy: the resolv.conf mode is stub, so applications resolve through systemd-resolved, and the global DNS points at the local loopback listener (`127.0.0.1:53053` or `[::1]:53053`), so systemd-resolved forwards to dnsproxy. When both hold, the surviving per-link provider servers are reported as a task warning with the remedy (remove the provider nameservers from the netplan or systemd-networkd configuration), and the task succeeds. When either fact is missing the system would bypass dnsproxy and the task reports the error as above.
