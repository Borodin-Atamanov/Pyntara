...
pyntara-wayrecord is a capture source: the ffmpeg CLI cannot capture Wayland natively, so the script asks the xdg-desktop-portal ScreenCast portal for a PipeWire stream (a file descriptor plus the stream node id), reads that stream with the GStreamer pipewiresrc element and writes raw frames to stdout. The caller pipes the stream into ffmpeg and controls every encoding parameter, for example:

pyntara-wayrecord | ffmpeg -f rawvideo -pix_fmt yuv420p -s 1920x1080 -r 30 -i pipe:0 -c:v libx264 out.mp4

The stream format is one GStreamer caps string given with --caps (default video/x-raw,format=I420,framerate=30/1, native size at 30 fps); the script passes it to the pipeline unchanged, so any pixel format, size and frame rate is possible, from 120 fps to one frame per ten seconds (framerate=1/10). The resolved geometry is printed to stderr so the caller can match -pix_fmt, -s and -r; stdout carries only raw frames. On the first run the KDE portal shows the screen-choice dialog once; the script then saves the single-use restore token the portal returns and passes it back on later runs, so the recording starts without asking again. The token lives in the per-user file wayrecord_token under the pyntara config directory, or in the path of the PYNTARA_WAYRECORD_TOKEN environment variable. The capture stops when the pipe closes (ffmpeg exits) or on Ctrl+C; --help shows examples.
