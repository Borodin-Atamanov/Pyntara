...
The telemetry_password entry carries the password that encrypts the System Metrics PDF reports: the encrypted PDF generation reads it when it produces a report. The entry exists in every vault. Its password differs between the vaults by design: the default vault carries the generated form, 4 proquint words joined by dashes, produced by the regeneration tooling; the production vault carries 4 proquint words joined by spaces, set once by hand after the entry was created, so the two vaults differ both in the separator and in the value.

Two optional data subgroups are part of the structure: NextDNS and port_forwarding_servers. Both are data, not structure: the regeneration tooling creates the groups, fills each with its configured seed entries on creation and never fills or deletes entries afterwards, so the accounts and server addresses survive every regeneration run. The port_forwarding_servers group carries one entry per port-forwarding server with the address in the url field; the ssh_passphase_for_port_forwarding entry carries the passphrase of the port-forwarding private key. Both the group and the entry exist in every vault: the default vault carries the seed entry with the well-known test address 200:a804:881c:d5d8:6d4e:afab:e158:371 and a freshly generated 7-proquint-word passphrase that unlocks no deployed key, so a runtime vault derived from it mirrors the production structure while connecting to nothing real.
