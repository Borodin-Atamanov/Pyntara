...
4. After a change the effective configuration is verified with ssh -G, which prints the result of the whole Include chain, so a directive that a later file overrides, or a keyword the client does not know, is reported as an error instead of being silently accepted.
