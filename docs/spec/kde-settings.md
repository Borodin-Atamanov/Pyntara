...
kwin_scripts_dir_name, desktop_ids_script_file_name and konsole_profile_file_name, the names of the task data files of the clone: the directory that carries one directory per KWin script, the python client that prints the live virtual desktop ids, and the Konsole profile. The task reads each one under task_data/kde_settings and reports a missing file instead of continuing.  
