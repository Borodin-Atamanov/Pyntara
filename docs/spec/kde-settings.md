...
kwin_scripts_dir_name, desktop_ids_script_file_name, konsole_profile_file_name and kglobalaccel_release_script_file_name, the names of the task data files of the clone: the directory that carries one directory per KWin script, the python client that prints the live virtual desktop ids, the Konsole profile, and the python client that asks the running KGlobalAccel daemon to release the hotkeys a script owns. The task reads each one under task_data/kde_settings and reports a missing file instead of continuing.  
