...
The external port is the desired port of the machine, derived from the hostname by the same function the port_forwarding task uses over the declared range of the port_forwarding_setup values, so both schemes name the machine with the same predictable number and an operator learns one value per machine.
...
A rule belongs to this machine when its description is the configured one, upnp_mapping_description, rendered by putting the hostname of the machine in place of its `{hostname}` placeholder. The router keeps the description of every rule and prints it back in its rule table, and that is the only ownership mark the oldest standard offers.

The mark names the machine because one router can carry the rules of two machines of this project at once: the rule of a neighbour carries the hostname of that neighbour, so it is never read as this machine's. The service never touches a rule of another machine. That matters because the router of the oldest standard replaces a rule silently instead of answering with an error: writing to a port is what takes it, so the decision to take a port must be deliberate, and the service sidesteps any port whose rule carries the mark of another machine.

A rule of this machine is written again when its target moved, which is what a changed address of the machine leaves behind, and when its mark is older than the configured one, which is what a description that gained the machine name leaves behind. In both cases the router takes the new rule over the old one, and both the address and the mark are derived again at every run.

## Telemetry

The System Metrics collector carries a upnp network module that runs the command pyntara.upnp_forwarding_state. The command reads the rules of the router live through the same client and reports one record per rule of this machine: the address of the router, the published port, the port of this machine the rule delivers to, the scope of the address and the ssh command that reaches the machine through it. The rules of other machines are left out, so the report carries the reachability of this machine alone.
...
The service wakes the report collector when, and only when, it changed the rule on the router: the mapping was missing, or it pointed at another address or another port, or it carried an older mark. That is the positive change a fresh report is worth, and the collector rebuilds the network report and the encrypted PDF, which the running System Metrics service then sends ([System Metrics](system-metrics.md), Availability changes).
...
A run of the service re-derives everything at every start: the desired port and the mark of a rule come from the values of the section, while the sshd listen port of this machine comes from the system config, and the address of the machine and the rule table come from the network. A change of the values therefore needs a new deployment, and a change of the sshd port, of the address of the machine or of the address of the router needs no step beyond the next run of the timer, which a forced task run or a manual run of the service makes immediate.
...
All parameters live in the module src/pyntara/values/upnp_forwarding_setup.py: the package that provides the client and the binary the service runs, the protocol and the description template of the rules this machine owns, whose `{hostname}` placeholder is filled with the hostname of the machine, the number of ports the service tries, the names of the two units and their templates, the module and the command line the oneshot unit runs, the systemctl calls of the task, the boot delay and the interval of the timer, the journal identifier, and the channel name with the two scope names the address carries in the report. The section declares no error priority: the service reports its trouble through the progress lines of the shared logger. The system config carries no copy of these values: the section has no config table. The port range and the port formula are not repeated here either: the service calls the shared function of pyntara.port_forwarding, which reads the declared range of the port_forwarding_setup values. The port of this machine is not configured at all: it is the sshd listen port of the ssh_daemon_setup directives, read through the shared ssh helper, so the rule and the daemon can never disagree.
