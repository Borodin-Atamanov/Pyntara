...
The host is resolved in this order, and the first available source wins: the public IPv4 address, the public IPv6 address, the yggdrasil node address, then the share address the panel already stores. The first two come from the shared `pyntara.public_address` helper: it queries all configured echo services in one parallel curl call, waits for every transfer, keeps the addresses that arrived and merges the repeats, so the same address reported by several services counts once and a slow service costs at most `server_ip_timeout_seconds` instead of hiding the other answers. A machine behind NAT has no public IPv4 address at all, so the IPv6 answer and the yggdrasil address keep it usable as a server; an address already stored in the panel is reused rather than overwritten with nothing. IPv6 goes into `shareAddr` wrapped in square brackets, which is exactly the form the panel stores and renders, so a rerun compares equal and stays a no-op. When none of the sources answers, the panel is left as it is and the stage reports a warning.
