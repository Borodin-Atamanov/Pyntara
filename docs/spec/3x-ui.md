...
`panel_geodata_domain_kind`, `panel_geodata_ip_kind` (strings, optional, defaults `"domain"`, `"ip"`): the kinds the panel accepts in its geodata validation request, for a domain token and for an address token.  
`inbound_sniffing_protocols` (array of strings, optional, default `["http", "tls"]`): protocols the panel is asked to detect on the universal inbound, so the routing rules can decide by the requested name.  
