...
Every source of an address is queried once per run, before the stages that need it, and the result is passed to every stage as run facts. One parallel echo-service call, one local-address call and, when UPnP is enabled, one router query produce the facts: the public addresses, the local addresses, the router address and the client-usable address the port forwarding returned. A machine without UPnP therefore pays for exactly one failed router query per run instead of one per stage, which keeps a run on a router-less network short.
