...
config.toml at the repository root is the single source of truth for the values used by the Python part: engine.task_data_root (task data root), engine.notice_timeout (seconds the resilience notice stays visible), engine.command_timeout_seconds (ceiling for provisioning commands), engine.process_check_timeout_seconds (bound for the desktop detection process query), engine.task_start_delay_seconds (pause between the task banner and the task start) and per-task sections such as cli_tools.packages, cli_tools.package_status_timeout_seconds, cli_tools.package_install_retries, cli_tools.package_success_threshold_percent, add_extra_repos.components and add_extra_repos.ubuntu_hosts (Ubuntu archive components and hosts managed by add_extra_repos), zram_service (compressor, swap priority, memory fraction, fallback CPU count, alignment), swapfile_service_install (swapfile path and size formula), local_vault_setup (runtime secret vault source and target paths, file modes and error priority, docs/spec/secrets-model.md) and system_metrics_setup (check interval of the long-running System Metrics service, docs/spec/telemetry.md). Module configuration is stored in config.toml: every task keeps its own section, so task parameters are configurable without code changes. The [[tasks]] section is the task catalog: one entry per task with name, description, dependencies and mode membership. The file is mandatory: a missing or invalid config stops the run, there are no defaults. Only the composition root reads the file; the values travel to tasks through Context.

Behavioral values must never be hardcoded inside task modules. A module-level constant is allowed only as a documented exception: system OS paths, repository layout paths, unit file names and kernel sysfs interfaces, which are not configuration but fixed machine contracts. Duplicating the same value or the same logic across modules is forbidden: shared values and helpers live in one module and are imported, never copied.
...
The System Metrics service is the first long-running systemd service deployed from the pyntara code base. The system_metrics_setup task installs the package into the dedicated virtual environment /usr/local/lib/pyntara/venv with uv from the repository clone and copies the repository config.toml to /etc/pyntara/config.toml, the single config of the target system: the deployed service reads it through pyntara.config.load_config, the same loader the installer uses, so both sides share one source of truth. The service unit system_metrics.service runs /usr/local/lib/pyntara/venv/bin/python -m pyntara.metrics /etc/pyntara/config.toml and every check_interval_seconds verifies that the runtime secret vault at /var/lib/pyntara/secrets/pyntara.vault, created by local_vault_setup, opens with the password from /etc/pyntara/pass; the outcome is journaled through the shared pyntara.logger functions with the journal identifier system_metrics. The password value never appears in any message.
