...
2. The NextDNS profile ID shape: exactly six lowercase hex digits, validated by pyntara.nextdns. It is a format invariant of the NextDNS service, not a behavior of the installer, so it stays in code; every other NextDNS value (IPv4 servers, IPv6 prefixes, the DoT endpoint pattern, the verification URL, the drop-in section header and the managed directives) lives in the [nextdns_setup_system_wide] config table.
