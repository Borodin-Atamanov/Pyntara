...
config/ (config.toml loading and validation for the sections still kept there)  
...
The values used by the Python part live in one module per task under src/pyntara/values/, and a task reads the values of its own module at the point of use. The TOML config/ directory at the repository root is being retired section by section: a section that is still there (engine.toml, cli_tools.toml, ...) is joined by pyntara.config.loader.render_config_source into a single document before parsing. The loader accepts both the directory form and a single file, so the deployed system config stays one file. The section map lives in [Config section map](../guides/project-structure.md#config-section-map); per-task parameters are documented in each spec's Parameters section (docs/spec/). A task whose section is migrated is configured in its values module and keeps no copy in TOML; a task that is not migrated yet keeps its own section in config.toml, so its parameters are configurable without code changes. The task catalog is not a config section: it lives in src/pyntara/values/tasks.py, one record per task with name, description, dependencies and mode membership, and the install mode vocabulary sits beside it in the same module. The file is not policed while the run works: a missing file, an unreadable file, broken TOML, a section nobody knows, a key nobody reads, an absent value and a value of an unexpected type never stop the run, never invent a value and never repair one. The task that needed a value reports what it could not do, and the run continues, so the failure limit sits at the task and not at the project. The same holds for the values of a migrated section: a values module that cannot be imported, and a values module that does not declare a name its task reads, are both reported in plain words and cost that task only, while the remaining tasks still run (tests/test_values_softness.py proves both). Every rule of the config lives in the test suite instead: tests/config_checks.py holds the checks that guard the shipped config/, tests/value_checks.py and tests/test_values.py hold the rules of the values, and tests/test_config_coverage.py applies the config checks to the repository config and to the test documents during development, so a broken config fails where the developer sees it, never on the target machine. Download and release query curl calls share the engine-wide curl_timeout_seconds (metadata queries) and curl_download_timeout_seconds (file downloads), curl_retries, curl_retry_delay_seconds, curl_connect_timeout_seconds and curl_retry_max_time_seconds: the per-attempt --max-time and --connect-timeout bound a single attempt, and the --retry count with --retry-all-errors, the configured --retry-delay and the --retry-max-time total bound make every failure retryable within a bounded window, so a slow or flaky network gets a long window and retries instead of a short fixed timeout, an unreachable host fails its attempt fast instead of stalling to the full per-attempt budget, and a transfer dropped after the connection is established is retried as well. Every task that installs a program from GitHub releases reads the latest release through pyntara/github_release.py, one shared reader instead of a copy per task, and its endpoint is the engine-wide github_latest_release_url with {repo} replaced by the repository of the task, so the five tasks cannot drift apart. Only the composition root reads the file; the values travel to tasks through Context.

Behavioral values must never be hardcoded inside a task body. Every value that affects behavior lives in the values package (src/pyntara/values/, one module per task) or, while that section is not migrated yet, in its config/ section: unit file names, journal identifiers, queue and spool directory names, file modes and paths, the owner pair of a file the run creates as root (root_owner_uid and root_owner_gid of the [engine] table), and external API contracts such as the Google web app deployment URL pattern. All variables and constants live there except explicit exceptions approved by the user, each approval recorded either under Approved exceptions in this document or as a type in the Exceptions section of [Config content](../spec/config-content.md). A hardcoded literal is allowed only as such a recorded user-approved exception; every other literal is a violation. A constant found outside a values module and outside config/ without a recorded approval is a violation to fix on discovery: move it into the values module of its task immediately, never leave it in place. Duplicating the same value or the same logic across modules is forbidden: shared values and helpers live in one module and are imported, never copied. The types of values that belong in config/ and the closed list of types that stay in code are in [Config content](../spec/config-content.md): a value of a listed type is a config value, a value of an exception type stays in code, and the reason is manageability and understanding rather than mutability, because a value hidden deep inside a module cannot be read, compared or changed in one place, and the config can. A given value is written once, in the values module of the task that owns it or in its config/ section while that section is not migrated, and read wherever it is needed; a copy in a second place is a defect. A value that is computed, for example a path under the repository root, is computed once in a shared place and is imported, never rebuilt in every module that needs it.
...
PYNTARA_FORCE_TASKS - space-separated task names that must rerun even when the target state is reached. Invalid names are reported and ignored. The keyword that forces every task of the resolved run set is force_all_keyword of the engine table, compared without case; task names are case-insensitive as well.  
PYNTARA_SKIP_APT_UPDATE - 1, true or yes skips the apt index refresh that cli_tools and add_extra_repos run before package operations. The answers that mean true are the environment_flag_true_values list of the engine table, compared without case, so a machine that spells its answers differently is answered in the config and not in the code. Omit it in real runs so the index stays fresh; set it for test or offline runs.  
...
The NextDNS profile ID shape: exactly six lowercase hex digits, validated by pyntara.nextdns. It is a format invariant of the NextDNS service, not a behavior of the installer, so it stays in code; every other NextDNS value (the vault group title, the profile ID file path and mode) lives in src/pyntara/values/nextdns_setup_system_wide.py.
...
A recoverable failure is never fatal: a task that could not perform a step reports it in warnings and still completes, and the runner converts a task that returns success=False or raises into a completed result carrying the reason in warnings. The run always continues with the remaining tasks, and the entry point counts the tasks with warnings and exits nonzero, so an incomplete configuration is visible to scripts without ever stopping the provisioning. No config condition stops the run either: a value that is not in the document reaches the task as an absent value, and the task reports it.

Task definitions live in src/pyntara/values/tasks.py (the values package, not the config). Each record has name, description, dependencies and mode membership; dependencies must name tasks listed earlier, which keeps default task sets ordered and rules out cycles. task_catalog.py holds only the logic that operates on the catalog: validate_mode, default_tasks, resolve.
...
config coverage tests (the shipped config passes every check, every section has a field, every key is read by its check, every config file contributes a table, each test copy mirrors the repository config)
