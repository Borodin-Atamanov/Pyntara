...
three_x_ui_xray_setup: enable SSL certificate setup (Let's Encrypt for IP, option 2). Set XUI_SSL_MODE=ip before installer runs. Before SSL setup, ensure port 80 is free: if occupied by x-ui, stop the service; if occupied by an unknown process, kill it (check how it works on real system before try to code). On rerun (installer skipped), add a separate stage that checks for existing cert via x-ui setting -getCert and runs acme.sh setup if absent.  
