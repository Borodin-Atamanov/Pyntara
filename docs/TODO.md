...
3x_ui_xray_setup stage 3: create the universal server inbound through the panel API. Create a VLESS inbound with REALITY on the configured port from the task config table; on a rerun find the existing inbound and skip. Tests cover the request payloads and the idempotent re-run.
