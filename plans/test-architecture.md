...
- [ ] Все сценарии из таблиц Layer 1 реализованы и проходят
- [ ] Все сценарии из таблиц Layer 2 реализованы и проходят
- [ ] `make test` (или `uv run pytest`) завершается с 0 ошибками
- [ ] `mypy --strict` проходит без ошибок
- [ ] `ruff` не выдаёт предупреждений
- [ ] Ни один тест не содержит реальных паролей production-секретов
- [ ] Тесты не требуют прав root (используют `PYNTARA_ROOT_EUID`)
- [ ] Все тесты имеют таймаут (через `pytest-timeout` или явный)