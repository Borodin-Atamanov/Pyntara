...
own credential that survives in the vault backup. The same vault entry
carries the machine RustDesk ID in its username field, so the operator
reads the ID and the password together from the vault backup. The
machine ID is also written to id_file_path, so the System Metrics
...
def _ensure_vault_credentials(
    ctx: Context, machine_id: str | None, force: bool
) -> tuple[str | None, str | None, bool]:
    """The permanent password and ID; returns (password, warning, changed).

    The password and the machine RustDesk ID live in the runtime vault
    entry named by vault_entry_title: the password in the password field,
    the ID in the username field. A normal run reuses the stored
    password, so the machine keeps it; force mode (or a missing entry)
    generates a fresh password of password_words proquint words joined by
    password_separator. The username is set to the current machine ID
    whenever the stored value differs, so an entry created before the ID
    was stored or a rerun after an identity change is brought up to date.
    The vault is saved only when the password was regenerated or the
    username changed, so an already current entry is left untouched. A
    vault that cannot be opened returns (None, warning, False): the
    password is not changed, so an existing access credential is never
    lost silently.
    """
    ...
    if kp is None:
        return (
            None,
            "runtime vault unavailable: rustdesk password and ID not stored",
            False,
        )
    ...
    if entry is not None and entry.password and not force:
        ...
        changed = True
    if machine_id and entry is not None and entry.username != machine_id:
        entry.username = machine_id
...
def task(ctx: Context) -> TaskResult:
    ...
    configured option is applied, the permanent password and the machine
    ID are stored in the runtime vault entry, the password is applied,
    and the machine ID file matches the current ID; the task then returns
    ...
    password, password_warning, vault_changed = _ensure_vault_credentials(
