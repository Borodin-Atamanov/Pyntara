...
class ThreeXuiXraySetupConfig:
    ...
    and the task checks for enabled and active; service_start_wait_seconds
    is the budget the unit gets to report active after an install,
    panel_listener_wait_seconds the budget the panel HTTP listener gets
    after the panel restarted, and readiness_check_delay_seconds the pause
    between two readiness checks. core_ready_wait_seconds is the budget
    the running core gets to answer a routing question after a template
    write, which is what separates a core that is still loading its
    geodata from a policy the core refused. panel_port is the
    ...
    asks about, remote_link_default_port is the port of a remote
    ...
    service_start_wait_seconds: int
    panel_listener_wait_seconds: int
    ...
    inbound_port: int
    route_test_port: int
    route_test_network: str
    route_test_protocol: str
